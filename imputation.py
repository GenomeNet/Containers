...
def get_default_output_dir():
    date_str = datetime.date.today().strftime("deepG_imputation_%Y%m%d_1")
    output_path = Path(date_str)
    counter = 1
    while output_path.exists():
        counter += 1
        date_str = datetime.date.today().strftime(f"deepG_imputation_%Y%m%d_{counter}")
        output_path = Path(date_str)
    return date_str
...
def run_podman(input_file, output_dir, use_gpu=False, batch_size=300, threshold = 0.5, verbose=False):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog="Example usage:\n"
               "  ./imputation.py --input /path/to/example.fasta --output /path/to/output_dir\n"
               "  ./imputation.py --input /path/to/example.fasta --output /path/to/output_dir --gpu\n"
               "  ./imputation.py --input /path/to/example.fasta --output /path/to/output_dir --verbose"
    )
    ...
    run_podman(args.input, args.output, args.gpu, args.batch_size, args.threshold, args.verbose)