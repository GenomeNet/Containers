...
def get_default_output_dir():
    date_str = datetime.date.today().strftime("deepG_crispr_%Y%m%d_1")
    output_path = Path(date_str)
    counter = 1
    while output_path.exists():
        counter += 1
        date_str = datetime.date.today().strftime(f"deepG_crispr_%Y%m%d_{counter}")
        output_path = Path(date_str)
    return date_str
...
def run_podman(input_file, output_dir, use_gpu=False, batch_size=200, verbose=False):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog="Example usage:\n"
               "  ./crispr.py --input /path/to/example.fasta --output /path/to/output_dir\n"
               "  ./crispr.py --input /path/to/example.fasta --output /path/to/output_dir --gpu\n"
               "  ./crispr.py --input /path/to/example.fasta --output /path/to/output_dir --verbose"
    )
    parser.add_argument("--input", required=True, help="Path to input file")
    parser.add_argument("--output", default=get_default_output_dir(), help="Path to output directory (default: current date, e.g., 'deepG_sorf_20210312_1')")
