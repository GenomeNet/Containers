...
def get_default_output_dir():
    date_str = datetime.date.today().strftime("deepG_sorf_%Y%m%d_1")
    output_path = Path(date_str)
    counter = 1
    while output_path.exists():
        counter += 1
        date_str = datetime.date.today().strftime(f"deepG_sorf_%Y%m%d_{counter}")
        output_path = Path(date_str)
    return date_str
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog="Example usage:\n"
               "  ./sorf.py --input /path/to/example.fasta --output /path/to/output_dir\n"
               "  ./sorf.py --input /path/to/example.fasta --output /path/to/output_dir --gpu\n"
               "  ./sorf.py --input /path/to/example.fasta --output /path/to/output_dir --verbose"
    )
